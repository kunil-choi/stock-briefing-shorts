...
# 쇼츠 전용 세로 캔버스(9:16, YouTube Shorts 표준). step1/step2는 1920x1080
# 가로형이라 여기서 재정의한다 — html_theme.py/render.py는 이 상수를 그대로
# 읽어 렌더링하므로 이 파일 밖의 코드는 손댈 필요가 없다.
W, H = 1080, 1920

SUBTITLE_ZONE_TOP = 1580  # 자막 전용 영역 시작 Y좌표(하단 340px 고정 여백). 콘텐츠(카드/사진)는
